"""Aggregate stochastic single-task SAC baseline curves.
...
The formal Forward Transfer protocol uses stochastic evaluation curves.
Deterministic curves remain optional diagnostics and may be absent when
deterministic evaluation is disabled.
"""
...
def aggregate_single_task_baselines(
    *,
    ...
    det_eval_episodes = int(batch_config.get("det_eval_episodes", 1))
    stoch_eval_episodes = int(batch_config.get("stoch_eval_episodes", 1))

    if det_eval_episodes < 0:
        raise ValueError("det_eval_episodes must be non-negative.")
    if stoch_eval_episodes <= 0:
        raise ValueError("stoch_eval_episodes must be positive.")
    ...
    for task_index, task_name in enumerate(tasks):
        ...
        if task_steps != expected_steps:
            raise ValueError(
                "All tasks must use the same evaluation step grid. "
                f"Task {task_name!r} has {task_steps}, expected {expected_steps}."
            )

        stochastic_success = [
            _required_success(row, "stochastic_success_rate")
            for row in evaluation_rows
        ]
        stochastic_return = [
            _required_finite_float(row, "stochastic_average_return")
            for row in evaluation_rows
        ]
        stochastic_success_curves.append(stochastic_success)
        stochastic_return_curves.append(stochastic_return)

        deterministic_success = _optional_float_curve(
            evaluation_rows,
            "deterministic_success_rate",
            success=True,
        )
        deterministic_return = _optional_float_curve(
            evaluation_rows,
            "deterministic_average_return",
            success=False,
        )
        if deterministic_success is not None and deterministic_return is not None:
        ...
        selected_count = min(tail_size, len(stochastic_success))
        ...
        task_summary_rows.append(
            {
                ...
                "final_deterministic_success": (
                    "" if deterministic_success is None else deterministic_success[-1]
                ),
                "tail_mean_deterministic_success": (
                    "" if deterministic_success is None else _mean(
                        deterministic_success[-selected_count:]
                    )
                ),
                "maximum_deterministic_success": (
                    "" if deterministic_success is None else max(deterministic_success)
                ),
                "final_deterministic_return": (
                    "" if deterministic_return is None else deterministic_return[-1]
                ),
                "tail_mean_deterministic_return": (
                    "" if deterministic_return is None else _mean(
                        deterministic_return[-selected_count:]
                    )
                ),
                "final_stochastic_success": stochastic_success[-1],
                "final_stochastic_return": stochastic_return[-1],
    ...
    deterministic_success_json = (
        deterministic_success_curves
        if len(deterministic_success_curves) == len(tasks)
        else None
    )
    deterministic_return_json = (
        deterministic_return_curves
        if len(deterministic_return_curves) == len(tasks)
        else None
    )
    ...
    if deterministic_success_json is None:
        ...
        _write_wide_curve_csv(
            ...
            tasks=tasks,
            curves=deterministic_success_json,
        )
        _write_wide_curve_csv(
            path=deterministic_return_csv_path,
            evaluation_steps=expected_steps,
            tasks=tasks,
            curves=deterministic_return_json,
        )
    ...
    curves_payload: dict[str, Any] = {
        ...
        "det_eval_episodes": det_eval_episodes,
        "stoch_eval_episodes": stoch_eval_episodes,
        "max_episode_steps": int(batch_config.get("max_episode_steps", 200)),
        "evaluation_steps": expected_steps,
        "deterministic_success_curves": deterministic_success_json,
        "deterministic_return_curves": deterministic_return_json,
    ...
    summary_payload: dict[str, Any] = {
        ...
        "evaluation_steps": expected_steps,
        "tail_size": tail_size,
        "mean_final_stochastic_success": _mean(
            [curve[-1] for curve in stochastic_success_curves]
        ),
        "mean_tail_stochastic_success": _mean(
            [
                _mean(curve[-min(tail_size, len(curve)):])
                for curve in stochastic_success_curves
            ]
        ),
        "mean_final_stochastic_return": _mean(
            [curve[-1] for curve in stochastic_return_curves]
        ),
        "deterministic_evaluation_available": (
            deterministic_success_json is not None
        ),
        "stochastic_evaluation_available": True,
...
def _resolve_run_directory(
    *,
    ...
    raise FileNotFoundError(
        "Task run directory does not exist: "
...
def _validate_run_identity(
    *,
    ...
    checks: tuple[tuple[str, Any, Any], ...] = (
        ...
        (
            "det_eval_episodes",
            int(run_config.get("det_eval_episodes", det_eval_episodes)),
            det_eval_episodes,
        ),
        (
            "stoch_eval_episodes",
            int(run_config.get("stoch_eval_episodes", stoch_eval_episodes)),
            stoch_eval_episodes,
        ),
    )
