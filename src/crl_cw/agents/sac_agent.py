"""Soft Actor-Critic agent for the ClonEx-compatible multi-head baseline.

The actor and critics use shared backbones with task-specific output heads.
For a CW10 sequence, each task
has an independent entropy coefficient log(alpha), selected from the task
one-hot vector appended to observations.
"""
...
class SACAgent(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Keras Adam uses epsilon=1e-7 by default.
        self.optimizer = Adam(
            chain(
    ...
    def alpha(self) -> torch.Tensor:
        """Backward-compatible alpha property for one-task agents."""
        if self.num_tasks != 1:
            raise RuntimeError(
    ...
    def alpha_value(self) -> float:
        """Backward-compatible alpha value for one-task agents."""
        if self.num_tasks != 1:
            raise RuntimeError(
                "This agent has multiple alphas. "
    ...
    def select_action_with_head(
        ...
        *,
        ...
        This method is intended for ClonEx-style best-return
        exploration, where previous actor heads are evaluated and
        rolled out on the current task.
        """
    ...
    def _prepare_update_batch(
        ...
        *,
        ...
        return (
            ...
            next_observations.to(self.device, dtype=torch.float32),
            dones.to(self.device, dtype=torch.float32),
        )
