...
This module implements the metric definitions used by Continual World:
...
def compute_forward_transfer(
    ...
    For task i:
        ...
        When the baseline mean success is 1.0, raw FT remains valid,
        while normalized FT for that task is returned as NaN.
    ...
    for task_index, (
        ...
        if (
            ...
            # No normalized FT is defined when the independent
            # baseline already has mean success 1.0, because the
            # remaining performance headroom is zero. Raw FT remains
            # valid, so only this normalized value is marked as NaN.
...
def compute_continual_metrics(
    *,
    ...
    """Compute the complete Continual World metric summary.
